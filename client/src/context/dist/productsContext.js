...
var ProductsProvider = function (_a) {
    ...
    react_1.useEffect(function () {
        axios_1["default"].get('http://localhost:8000/api/products').then(function (response) {
    ...
    var addToCart = function (product, quantity) {
        ...
        if (index >= 0) {
            ...
            setCart(__assign(__assign({}, cart), { products: __spreadArrays(currentProducts) }));
        }
        else {
            var newItemInCart = {
                id: product.id,
            ...
            setCart(__assign(__assign({}, cart), { products: __spreadArrays(cart.products, [newItemInCart]) }));
        }
    };
    console.log(cart);
    return (React.createElement(exports.ProductsContext.Provider, { value: { products: products, cart: cart, addToCart: addToCart } }, children));
};
